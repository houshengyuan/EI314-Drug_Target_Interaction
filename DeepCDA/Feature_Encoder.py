...
def feature_extraction_model(FLAGS, embedding_size, num_filters, protein_filter_lengths, smiles_filter_lengths):
    ...
    predictions = Dense(1,activation='sigmoid',kernel_initializer='normal', name='dense4')(FC2) # if you want train model for active/inactive set activation='sigmoid'
...
def crossfold_validation( flags):
    ...
    binary_accuracys = [[0 for x in range(fold_nums)] for y in range(h)] 
                ...
                for ind_3 in range(len(hyper_param_3)):
                    ...
                    for ind_4 in range(len(hyper_param_4)):
                         ...
                         for ind_5 in range(len(hyper_param_5)):
                             ...
                             binary_crossentropy,binary_accuracy,binary_precision,binary_recall,f1_score,true_positive,true_negative,false_positive,false_negative= model.evaluate(([np.array(val_drugs),np.array(val_proteins) ]), np.array(val_affinity), verbose=0)
                             ...
                             binary_accuracys[iterator][fold]=binary_accuracy
                             iterator=iterator+1
    ...
    mean_param_list=np.mean(np.array(binary_accuracys),axis=1)
    ...
    return losses, binary_accuracys
